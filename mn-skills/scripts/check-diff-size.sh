...
if [ "$CHANGED" -gt "$HARD" ]; then
  MSG="Diff is $CHANGED lines — over the 600-line hard budget. Split this tranche (mn-skills:pr-open)."
  if [ "$CI" = "--ci" ]; then echo "::error::$MSG"; else echo "HARD FAIL: $MSG"; fi
  exit 1
elif [ "$CHANGED" -gt "$SOFT" ]; then
  MSG="Diff is $CHANGED lines — over the 400-line soft budget. Consider splitting (mn-skills:pr-open)."
