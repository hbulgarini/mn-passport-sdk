...
- `mn-skills:conformance` finds the code diverging from the docs for a
  *good* reason.
- `mn-skills:verify` produces a "does it run" finding that contradicts a
...
4. **Sweep the verify register** (`.mn-skills/verify-register.md`): mark
   entries this learning closes as resolved (in place, dated); add new open
   items it surfaces. This register is owned here, fed by `mn-skills:verify`.
5. **Keep the derived layers honest**: if the change invalidates part of the
   `mn-skills:conformance` checklist, update that skill in the same change —
