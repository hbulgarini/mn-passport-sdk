...
```bash
bash mn-skills/scripts/check-diff-size.sh   # same script and numbers as CI
```
...
- Confirm nothing from `.mn-skills/` or any register/secret material is
  staged.
...
review and merge; `mn-skills:spec-driver` then updates `STATE.md` and moves
to the next tranche.
