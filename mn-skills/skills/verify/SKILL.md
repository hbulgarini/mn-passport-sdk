...
3. **Run it for real**, leaning on the repo's installed tooling rather than
   ...
   - `midnight-tooling:devnet` / proof-server skills for infrastructure;
     `mn-skills:devenv` first if anything is down.
...
`.mn-skills/verify-register.md` — persistent and appended, owned jointly
with `mn-skills:doc-sync` (which sweeps it for re-checks). Entry format:
