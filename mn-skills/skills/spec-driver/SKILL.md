...
The spine of the mn-skills workflow (`docs/development-workflow.md` §2–3).
...
2. Consult the `mn-skills:conformance` rules so the plan aligns with the
...
3. Break the spec into an **ordered** list of tranches. For each record:
   ...
   - **Acceptance gate** — an observable criterion (compiles, named test
     passes, flow runs end-to-end via `mn-skills:verify`).
...
a. `mn-skills:devenv` — confirm the environment is ready (includes the
   debts-repo check).
...
c. Implement the tranche. If implementation reveals the estimate was wrong
   ...
   do not plough on.
d. Run the four lenses — `mn-skills:conformance`, `mn-skills:security-audit`,
   `mn-skills:code-style`, `mn-skills:verify` — in parallel via subagents
...
e. If conformance found the code diverging from the docs for a *good* reason
   → `mn-skills:doc-sync` (update docs + ADR). The docs are corrected, not
   the code bent to a stale doc.
f. `mn-skills:pr-open` — prepare branch, commits, and description, then
