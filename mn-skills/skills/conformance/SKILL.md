---
name: conformance
description: Design-conformance review lens for Midnight Passport SDK changes — checks code against docs/sdk-requirements.md and docs/architecture.md (package dependency rules, kernel secret boundary, normative MUSTs, naming, version axes). Use after implementing a tranche, at plan time for alignment, or whenever asked to check a change against the design docs.
---

# conformance — the design guard

Checks a change against the source of truth: `docs/sdk-requirements.md`
(§ references below) and `docs/architecture.md` (arch §). **The docs win over
this checklist** — it is derived from them (as of 2026/07/27); if they have
moved, re-derive from the docs and update this skill via `mn-skills:doc-sync`.
...
`mn-skills:doc-sync`; never bend correct code to a stale doc).
